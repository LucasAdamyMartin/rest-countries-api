...
export function Country({country} : Readonly<countryProps>) {
    return( 
        <a href={`/rest-countries-api/${country.name.replace(/\s+/g, '-').toLowerCase()}`} key={country.name} className="flex flex-col justify-center items-center shadow-md w-[275px] rounded-lg dark:bg-darkBlue dark:text-white overflow-hidden">
